...
// ---------Расширения--------------------------
	КолРасширений = КоличествоРасширений();
	Если КолРасширений = 0 Тогда // нет расширений !
		Элементы.ГруппаРасширения1.Видимость = Ложь
	Иначе
		СоздатьЭлементыФормыДляРасширений(КолРасширений);
		Для Индекс = 0 По КолРасширений-1 Цикл
			Номер = строка(Индекс+1);
			ДеревоОбъектовМетаданныхРасширенияЗаполнить(Индекс);
			Если ЭтаФорма["РодительскиеПодсистемы"+Номер].Количество() > 0 Тогда
				Элементы["ДеревоОбъектовМетаданных"+Номер].НачальноеОтображениеДерева = НачальноеОтображениеДерева.РаскрыватьВсеУровни;
			КонецЕсли;
			НачальнаяПометкаКоллекций( ЭтаФорма["ДеревоОбъектовМетаданных"+Номер] );
		КонецЦикла;
	КонецЕсли;
...
Процедура ОтметитьПоПодсистемам(Команда)
	...
	ПараметрыФормы = Новый Структура("ИмяДереваОбъектовМетаданных", "ДеревоОбъектовМетаданных");
...
Процедура ОтметитьПоПодсистемам1(Команда)
	...
	ПараметрыФормы = Новый Структура("ИмяДереваОбъектовМетаданных", "ДеревоОбъектовМетаданных1");
	ОткрытьФорму("ВнешняяОбработка.ДиаграммаОбъектов1С.Форма.ОтборПоПодсистемам", ПараметрыФормы, ЭтаФорма, 
		УникальныйИдентификатор, , , Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ДеревоОбъектовМетаданныхПометкаПриИзменении(Элемент)
	...
	//// поиск элемента в первом попавшемся расширении
	ЭлементДереваРасширения = НайтиЭлементВДругомДереве(ТекущиеДанные, Ложь);
	Если ЭлементДереваРасширения <> Неопределено Тогда
		
		ЭлементДереваРасширения.Пометка = ТекущиеДанные.Пометка;
		ПометкаПриИзменении(ЭлементДереваРасширения);
...
Процедура ДеревоОбъектовМетаданных1ПометкаПриИзменении(Элемент)
	...
	// поиск и пометка элементов в основной конфигурации
	ЭлементДереваКонфигурации = НайтиЭлементВДругомДереве(ТекущиеДанные, Истина);
	Если ЭлементДереваКонфигурации <> Неопределено Тогда
		
		ЭлементДереваКонфигурации.Пометка = ТекущиеДанные.Пометка;
		ПометкаПриИзменении(ЭлементДереваКонфигурации);
...
Процедура ОповещениеОЗавершенииОтбора(РезутатОтбора, ДополнительныеПараметры) Экспорт
	...
	ОтметитьПоОбъектамРекурсивно(ЭлементыКлассовОбъектовМетаданных, СписокОбъектов);
	
	Если ИмяДереваОбъектовМетаданных <> ИмяДереваОбъектовМетаданных0 Тогда
		ЭлементыВерхнегоУровня = ЭтаФорма[ИмяДереваОбъектовМетаданных].ПолучитьЭлементы();
		ЭлементыКлассовОбъектовМетаданных = ЭлементыВерхнегоУровня[0].ПолучитьЭлементы();
		ОтметитьПоОбъектамРекурсивно(ЭлементыКлассовОбъектовМетаданных, СписокОбъектов);
	КонецЕсли;
...
// ------ Программное добавление эл-тов формы для Расширений ----------
&НаСервере
Процедура СоздатьЭлементыФормыДляРасширений(КолРасширений)
	Если КолРасширений=1 Тогда Возврат КонецЕсли;
	
	Для Номер = 2 по КолРасширений Цикл
		// создаём деревоЗначений ДеревоОбъектовМетаданных1
		// создаём СписокЗначений РодительскиеПодсистемы1
		// команду ОтметитьПоПодсистемам1(Команда)

		// создаём элемент ГруппаРасширения1
		// создаём элемент ДеревоОбъектовМетаданных1
		// создаём элемент ДеревоОбъектовМетаданных1ОтметитьПоПодсистемам1
	КонецЦикла;
...
Функция ИменаГруппМетаданныхДляВыбора()
	стрГруппы = "ОбщиеМодули,Роли,ПланыОбмена,WebСервисы,HTTPСервисы" // +ПараметрыСеанса,ОбщиеФормы,ОбщиеМакеты
...
Функция ТаблицаОбъектовМетаданных()
	...
	НоваяСтрокаТЗ("HTTPСервисы",                  "HTTP-сервисы",                   71, 72, Истина, ТЗ);
...
Процедура ДеревоОбъектовМетаданныхЗаполнить()
	...
	ПараметрыЭлемента = НовыйЭлементДереваОбъектовМетаданных(Метаданные.Имя, Метаданные.ПолноеИмя(), Метаданные.Синоним,
	...
	Для Каждого Строка Из КоллекцииОбъектовМетаданных Цикл
		...
		Если КоллекцияВыбираемыхОбъектовМетаданных.Количество() = 0
			...
			ДобавитьЭлементДереваОбъектовМетаданных(Строка, ?(Строка.Имя = "Подсистемы", Метаданные.Подсистемы, Неопределено), Метаданные);
		КонецЕсли;
	КонецЦикла;
...
//
&НаСервере
Процедура ДеревоОбъектовМетаданныхРасширенияЗаполнить( ИндексРасширения = 0 )
	...
	// Создание предопределенных элементов.
	ПараметрыЭлемента = НовыйЭлементДереваОбъектовМетаданных(МетаданныеРасширения.Имя, МетаданныеРасширения.ПолноеИмя(), МетаданныеРасширения.Синоним,
															ЭтаФорма["ДеревоОбъектовМетаданных"+Строка(ИндексРасширения+1)], 79, 79);
	ЭлементКонфигурация = НоваяСтрокаДерева(ПараметрыЭлемента);
	ПараметрыЭлемента = НовыйЭлементДереваОбъектовМетаданных("Общие", "Общие", "Общие", ЭлементКонфигурация, 80, 80);
	ЭлементОбщие = НоваяСтрокаДерева(ПараметрыЭлемента);
	...
	Для Каждого Строка Из ГруппыОбъектовМетаданных Цикл
		...
		Если МетаданныеРасширения[Строка.Имя].Количество() = 0 Тогда // нет объектов данной группы в расширении
			Продолжить;
		КонецЕсли;
		...
		Если КоллекцияВыбираемыхОбъектовМетаданных.Количество() = 0
			Или КоллекцияВыбираемыхОбъектовМетаданных.НайтиПоЗначению(Строка.Имя) <> Неопределено Тогда
			Строка.Родитель = ?(Строка.ЭтоКоллекцияОбщие, ЭлементОбщие, ЭлементКонфигурация);
			ДобавитьЭлементДереваОбъектовМетаданных(Строка, ?(Строка.Имя = "Подсистемы", МетаданныеРасширения.Подсистемы, Неопределено), МетаданныеРасширения);
		КонецЕсли;
	КонецЦикла;
...
//
&НаСервере
Функция ДобавитьЭлементДереваОбъектовМетаданных(ПараметрыЭлемента, Подсистемы = Неопределено, МетаданныеКонфИлиРасширения = Неопределено, Проверять = Истина)
	...
	НоваяСтрока = НоваяСтрокаДерева(ПараметрыЭлемента, Подсистемы <> Неопределено И Подсистемы <> МетаданныеКонфИлиРасширения.Подсистемы);
	...
	Для Каждого Эл Из Подсистемы Цикл
		Если (Не Проверять
			...
			ПараметрыЭлемента = НовыйЭлементДереваОбъектовМетаданных(Эл.Имя, Эл.ПолноеИмя(), Эл.Синоним,
...
&НаКлиенте
Функция НайтиЭлементВДругомДереве(Знач ЭлементПоиска, ПоискВДеревеКонфигурации = Истина)
	
	ЭлементВДругомДереве = Неопределено;
	
	Если ЭлементПоиска = Неопределено Тогда // ДанныеФормыЭлементДерева / Неопределено
		Возврат ЭлементВДругомДереве;
	КонецЕсли;
	
	ЭлИмя = ЭлементПоиска.Имя;
	
	Если ПоискВДеревеКонфигурации Тогда
		Дерево = ЭтаФорма.ДеревоОбъектовМетаданных;
		ЭлементВДругомДереве = НайтиЭлементВДереве(ЭлИмя, Дерево);
	Иначе
		КолРасширений = КоличествоРасширений();
		Для н = 1 по КолРасширений Цикл
			ДеревоРасширения = ЭтаФорма["ДеревоОбъектовМетаданных" + строка(н)];
			ЭлементВДругомДереве = НайтиЭлементВДереве(ЭлИмя, ДеревоРасширения);
			Если ЭлементВДругомДереве <> Неопределено Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Функция НайтиЭлементВДереве(ЭлИмя, РодительскийЭлементДерева)
	
	ЭлементДерева = Неопределено;
	
	Если РодительскийЭлементДерева = Неопределено Тогда // ДанныеФормыЭлементДерева
		Возврат ЭлементДерева;
	КонецЕсли;
	
	ЭлементыДерева = РодительскийЭлементДерева.ПолучитьЭлементы();
	
	Для каждого Эл Из ЭлементыДерева Цикл
		
		Если Эл.Имя = ЭлИмя Тогда // на этом уровне
			ЭлементДерева = Эл;
			Прервать;
		КонецЕсли;
		
		ЭлементДерева = НайтиЭлементВДереве(ЭлИмя, Эл); // рекурсивно вниз
...
//Получаем массив метаданных расширений
	...
	Для Каждого ЭлементСписка Из ВыбранныеОбъектыМетаданных Цикл
		...
		Если МетаданныеОбъекта = Неопределено Тогда // +++ OS 02.10.2024 поиск по расширениям!
			Для Каждого ЭлИета Из МассивМетаданныхРасширений Цикл
				МетаданныеОбъекта = ЭлИета.НайтиПоПолномуИмени(ЭлементСписка.Значение);
...
Функция СформироватьСтрокуМетаданных(Команда, МетаданныеОбъекта, СтандартнаяСхема = Истина)
	...
	Если МетаданныеОбъекта = Неопределено  Тогда
		Возврат ""; // нет внутренней структуры, это группа Общие или иная
	КонецЕсли;
