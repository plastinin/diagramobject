...
//   Подсистемы      - ОбъектМетаданныхКонфигурация  - если заполнен, то содержит значение 
//   Проверять       - Булево - признак проверки на принадлежность родительским подсистемам.
// 
// Возвращаемое значение - СтрокаДереваЗначений - Строка дерева объектов метаданных.
//
&НаСервере
...
Функция ПараметрыЭлементаДереваОбъектовМетаданных()
	...
	СтруктураРезультат.Вставить("Синоним", "");
	СтруктураРезультат.Вставить("Пометка", Ложь);
	СтруктураРезультат.Вставить("Картинка", 0);
	СтруктураРезультат.Вставить("КартинкаОбъекта", 0);
	СтруктураРезультат.Вставить("Родитель", Неопределено);
...
// Картинка      - Картинка - картинка поставленная в соответствие группе метаданных
//                 или виду объекта метаданных.
// КартинкаОбъекта - Картинка - картинка поставленная в соответствие объекту метаданных  
// ЭтоКоллекцияОбщие - Булево - признак того, что текущий элемент содержит подэлементы.
// Таб			- ДеревоЗначений - Дерево объектов метаданных
//
&НаСервере
...
Процедура ДополнитьОписаниемКоллекции(СтрокаМетаданные, МетаданныеОбъекта, СборкаВзаимосвязей, ВыбранныеОбъектыМетаданных, ПутьКоллекция = "Реквизиты")
	...
	Если ВыводитьПредопределенныеРеквизиты Тогда
		ДополнитьОписаниеЭлементаПоКоллекции(МетаданныеОбъекта.СтандартныеРеквизиты, МетаданныеОбъекта, СборкаСтроки, СборкаВзаимосвязей, ВыбранныеОбъектыМетаданных, ПутьКоллекция, "{static} ");	
	КонецЕсли;
	...
	ДополнитьОписаниеЭлементаПоКоллекции(Коллекция, МетаданныеОбъекта, СборкаСтроки, СборкаВзаимосвязей, ВыбранныеОбъектыМетаданных, ПутьКоллекция);
	...
	Если СтрНайти(СтрокаМетаданные, "Справочник.")
		...
		Для Каждого ТабличнаяЧасть Из МетаданныеОбъекта.ТабличныеЧасти Цикл
			...
			ДополнитьОписаниеЭлементаПоКоллекции(ТабличнаяЧасть.Реквизиты, МетаданныеОбъекта, СборкаСтроки, СборкаВзаимосвязей, ВыбранныеОбъектыМетаданных, ПутьКоллекция);
		КонецЦикла;  
...
&НаСервере
Процедура ДополнитьОписаниеЭлементаПоКоллекции(Коллекция, МетаданныеОбъекта, СборкаСтроки, СборкаВзаимосвязей, ВыбранныеОбъектыМетаданных, ПутьКоллекция, Префикс = "+")
	...
	Для Каждого Элемент Из Коллекция Цикл
		...
		Если ПутьКоллекция <> "ЗначенияПеречисления" Тогда
			ДополнитьВзаимосвязи(Элемент.Имя, Элемент.Тип.Типы(), СборкаВзаимосвязей, МетаданныеОбъекта, ВыбранныеОбъектыМетаданных); 
		КонецЕсли;
		...
		Если ЭтоСсылка(ТипДанных) Тогда
			...
			Если ВыбранныеОбъектыМетаданных.НайтиПоЗначению(МетаданныеСвязи.ПолноеИмя()) <> Неопределено Тогда   
				...
				Если ФормироватьДетальныеОтношенияНаУровнеРеквизитов Тогда
					СборкаВзаимосвязей.Добавить(СтрШаблон("%1 --> %2::%3", МетаданныеСвязи.Имя, МетаданныеОбъекта.Имя, Имя));
				Иначе
