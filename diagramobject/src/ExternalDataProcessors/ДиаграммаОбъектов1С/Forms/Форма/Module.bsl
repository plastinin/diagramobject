...
Функция СгенерированноеИзображение(Данные, Параметры) 
	...
	ИзображениеСформировано = КодСостояния = 200;
	
	Если ИзображениеСформировано Тогда
		
		ДвоичныеДанныеКартинки = Ответ.ПолучитьТелоКакДвоичныеДанные();
		Результат = Новый Структура;
		Результат.Вставить("ОтветСервера", Ответ);
		Если Параметры.ВыводитьВФорматеHTML Тогда
	...
	Иначе 
		//Анализируем ошибки в ответе сервера
		ПревышенЛимитОбъектовНаСервереРендеринга = КодСостояния = 403;
		
		СтрокаОтвета = Ответ.ПолучитьТелоКакСтроку();
		
		Если ПревышенЛимитОбъектовНаСервереРендеринга Тогда
		
		ВызватьИсключение СтрШаблон("ОШИБКА: Превышен лимит объектов на сервере рендеринга %1, Код ошибки: %2: %3",
													 Параметры.СерверРендеринга, КодСостояния, СтрокаОтвета);
		Иначе
			
		ВызватьИсключение СтрШаблон("ОШИБКА! Код ошибки: %1: %2", КодСостояния, СтрокаОтвета);
...
Функция ПараметрыПоУмолчанию()
	...
	Результат.Вставить("ВыводитьВФорматеHTML", Ложь);
...
Процедура СгенерироватьКодНаСервере()
	...
	Элементы.ДекорацияШаг3.Доступность = Истина;
...
>>>>>>> ffff7dc62184c1ef98b2b24749c7383eb05eabb5
